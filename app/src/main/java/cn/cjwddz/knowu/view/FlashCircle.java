...
public class FlashCircle extends View{
...
//    private static final int RED = 230, GREEN = 85, BLUE = 35; //基础颜色，这里是橙红色
     ...
     * @param isR
     */
    public void SetRotating(boolean isR){
        curMinAlpha = isR?MIN_ALPHA:MAX_ALPHA;
    ...
    public FlashCircle(Context context) {
        super(context);
        init();
    }

    public FlashCircle(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public FlashCircle(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private int calculateWaveAlpha(float waveRaduis){
        ...
        if(percent >= 1f){
            ...
            return (int) (curMinAlpha*(1f-percent));
        }
    }
